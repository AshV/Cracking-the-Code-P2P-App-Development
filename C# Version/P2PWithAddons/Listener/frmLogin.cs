namespace Listener
{
	...
	public delegate void StreamingCallBackDeligates(string e,StreamIt str);
...
//		private Socket newSock;
			...
			try
			{
				if( 0 != textLoginID.Text.Trim().Length )
				{
					...
					{*/
						...
						AcceptThread.Start();
						threadIndex[ThreadCount] = (object)AcceptThread;
						++ThreadCount;
						...
						ThreadIcon.Start();
						threadIndex[ThreadCount] = ThreadIcon;
						++ThreadCount;
			...
			try
			{
				while(true)
				{
					...
					else if(bStreaming)
					{
						if((IconNo) == 5)
							IconNo = 1;
						Filename = "\\icon" + IconNo.ToString() + ".ico";
						ListenIcon.Text = "Streaming in Progress : " + connectedStreamingClients.ToString() + " Clients Connected";
		...
		protected void AcceptFunction()
		{
			try
			{
				while(true)
				{
					...
					RespondingThread.Priority = ThreadPriority.AboveNormal;
					RespondingThread.Start();
					threadIndex[ThreadCount] = RespondingThread;
					++ThreadCount;
				}
		...
		public void StreamingCallBack(string e,StreamIt source)
		{
			Application.DoEvents();
			if((e.CompareTo("Stopped") == 0))
			{
				if(bStreaming)
					new XMLCreater(Application.StartupPath + "\\Temp1.xml","").WriteErrorResponse("A streaming session is already active on this peer");
				else
				{
					new XMLCreater(Application.StartupPath + "\\Temp1.xml","").WriteErrorResponse("Problem encountered while initiating the Streaming Session");
					bStreaming = false;
				}
			}
			else if((e.CompareTo("Encoder Stopped") == 0) || (e.CompareTo("Disconnected") == 0))
				bStreaming = false;
			else if(e.CompareTo("Copy Complete") == 0)
			{
				bStreaming = true;
				new XMLCreater(Application.StartupPath + "\\Temp1.xml","").WriteErrorResponse("No Error");
			}
			else
				return;
			if(bStreaming && (e.CompareTo("Connected") == 0))
				connectedStreamingClients = source.ConnectedClients;

			NetworkStream DataStream = new NetworkStream(newSock);
			Byte[] readBytes = new Byte[1024],read = new Byte[1024];
			FileStream fs1 = new FileStream(Application.StartupPath + "\\Temp1.xml",FileMode.Open);
			BinaryReader r = new BinaryReader(fs1);
			int nreadBytes;
			// write upon the network stream
			while( (nreadBytes = r.Read(readBytes, 0, readBytes.Length)) > 0) 
		...
		public void Logout ()
		{
			try
			{
				...
				for(int i = threadIndex.TotalThreads();i>=0;i--)
					if(threadIndex[i] !=null)
						if(((Thread)threadIndex[i]).IsAlive)
						{
							((Thread)threadIndex[i]).Abort();
							((Thread)threadIndex[i]).Join(2000);
						}
