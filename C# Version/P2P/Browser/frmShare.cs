		...
		public bool Parsing(string Filename)
		{
			...
			if( 0 == new ServerCommunication().TypeOfXMLRecieved(RESPONSEFILE).CompareTo("SHOWFILES") )
			{
				...
				if( 0 < iEntries )
				{
					...
					/// are to be sorted by there sizes
					...
					// A temorary array is maintained for the inner 
					...
					// declares an array of two string
					//string[] sSubItem = new string[2];
				...
				{
					// Calls the DisableUI function
					DisableUI();
					
					// Displays the Message Box 
					MessageBox.Show("No result found for this request","Warning",MessageBoxButtons.OK, MessageBoxIcon.Warning); 
				}
			}
